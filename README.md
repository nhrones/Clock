# Clock

The Dot-Clock Typescript code in the `src` folder is heavily commented.
I built this as a learning exercise for a grandson who was taking computer science at UGA.
The interesting bits was building a resource-pool of dots, to reduce V8 garbage collects.(/src/dotPool.ts)

Below the clock canvas are four slider controls. 
Try moving these sliders to change the animation.  Have fun!
 
Click the image below to check the time!
[![clock](https://github.com/nhrones/Clock/blob/master/clock.png)](https://nhrones.github.io/Clock/)
